...
const getOne = (id) => __awaiter(void 0, void 0, void 0, function* () {
    let response = yield database_1.default.query("SELECT * FROM cars WHERE id = $1", [id]);
    return response.rows[0];
});
exports.getCars = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let userInfo = yield verifyToken(req.headers.token);
        const response = yield database_1.default.query("SELECT * FROM cars WHERE created_by_user_id = $1", [userInfo.id]);
...
exports.getCar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    let userInfo = yield verifyToken(req.headers.token);
    let foundedCar = yield getOne(id);
    if (!foundedCar)
        return res.json({ message: "Car not found!" });
    let response = yield database_1.default.query("SELECT * FROM cars WHERE id = $1 AND created_by_user_id = $2", [id, userInfo.id]);
...
exports.createCar = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!req.headers.token)
        return res.json({ message: "Token not found !" });
    let userInfo = yield verifyToken(req.headers.token);
    const response = yield database_1.default.query(`INSERT INTO cars(title, price, color, description, created_by_user_id) 
    VALUES ($1, $2, $3, $4, $5)`, [title, price, color, description, userInfo.id]);
